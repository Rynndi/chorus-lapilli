...
# =========================== [ADD YOUR TESTS HERE] ===========================
    ...
    def test_no_pieces_after_three(self):
        '''check to see if after 3 moves by each player, it stops letting you put x's and o's down'''
        tiles = self.driver.find_elements(By.XPATH, self.BOARD_TILE_XPATH)
        #fill first two rows with X and Os
        for i in range(6):
            tiles[i].click()
        # Try to click on the remaining tiles and assert they are still blank
        for j in [6, 7, 8]:
            tiles[j].click()
            self.assertTileIs(tiles[j], self.SYMBOL_BLANK)
            
    def test_valid_x(self):
        '''after filling the first two rows,  test that 'O' can't move, but 'X' can move diagonally'''
    ...
    def test_prevent_moves_after_win(self):
        '''after making a move that would win, test to see that further moves cannot be made '''
        tiles = self.driver.find_elements(By.XPATH, self.BOARD_TILE_XPATH)
    
        move_sequence = [0, 3, 4, 1, 5, 2]
        ...
        winner_status = self.driver.find_element(By.XPATH, "//div[contains(text(),'Winner: X')]")
        self.assertTrue(winner_status.is_displayed(), "Winning status not displayed")
